...
async def _run_agent(runner: Runner, user_id: str, session_id: str, message: str) -> str:
    """Run a single agent and return its text response, with retry on 503."""
    for attempt in range(_RETRY_ATTEMPTS):
        await session_service.create_session(
            app_name=APP_NAME,
            user_id=user_id,
            session_id=f"{session_id}-attempt{attempt}",
        )
        ...
        try:
            ...
            if "503" in str(e) or "UNAVAILABLE" in str(e):
                ...
                log.warning("503 UNAVAILABLE, retrying in %ds (attempt %d/%d)...", delay, attempt + 1, _RETRY_ATTEMPTS)
